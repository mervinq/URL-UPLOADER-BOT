...
async def echo(bot, update):
    ...
    if "drive.google.com" in url:
        try:
            await imog.edit_text(Translation.DOWNLOADING)
            logger.info(update.from_user.first_name)
            await DownLoadFile(update.from_user.id, url, file_name, imog, bot, update)
        except Exception as e:
            logger.error(str(e))
            await imog.edit_text(Translation.SOMETHING_WRONG)
    else:
        await imog.edit_text(Translation.NOT_SUPPORTED)
...
ikeyboard = [
    InlineKeyboardButton(
        "🎥 SVideo [" + format_string + "] ( " + approx_file_size + " )",
        callback_data=cb_string_video.encode("UTF-8")
    ),
    InlineKeyboardButton(
        "📁 DFile [" + format_string + "] ( " + approx_file_size + " )",
        callback_data=cb_string_file.encode("UTF-8")
    )
]
inline_keyboard.append(ikeyboard)
